...
### Eos_H2 vs. Eos_H1
## run GSEA 
ranks <- ranks_for_fGSEA("/scratch/khandl/eos_human/data_files/DEGs/eos_cond/Eos_H2_vs_Eos_H1.csv")
fGSEA_to_csv(ranks, "/scratch/khandl/eos_human/data_files/GSEA/human_eos_Eos_H2_vs_Eos_H1.csv")

## plot GO terms of interest 
df <- read.csv("/scratch/khandl/eos_human/data_files/GSEA/human_eos_Eos_H2_vs_Eos_H1.csv")
...
p <- ggplot(df, aes(x = NES, y = pathway, size = padj,color = padj)) + 
  geom_point() + scale_size(name = "padj", range = c(3, 18)) + theme_light()
ggsave("/scratch/khandl/eos_human/figures/GO_Eos_H2_vs_Eos_H1.svg", width = 12, height = 6, plot = p)
